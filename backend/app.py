...
class ChatMessage(db.Model):
    ...
    # Pour faciliter l'affichage JSON
    def to_dict(self):
        return {
            "id": self.id,
...
def etudiant_notes():
    ...
    for n in notes:
        notes_par_matiere[n.matiere].append(n)
    for matiere, ns in notes_par_matiere.items():
        tp = sum(n.valeur * n.coefficient for n in ns)
        tw = sum(n.coefficient for n in ns)
        moyennes_par_matiere[matiere] = round(tp / tw, 2) if tw else 0
    ...
    toutes_notes = Note.query.order_by(Note.id).all()
    moyennes_classe = []
    for n_eleve in notes_triees:
        notes_avant = [n for n in toutes_notes if n.id <= n_eleve.id]
        tp = sum(n.valeur * n.coefficient for n in notes_avant)
        tw = sum(n.coefficient for n in notes_avant)
        moyennes_classe.append(round(tp / tw, 2) if tw else 0)

    # C'est ici que l'alignement doit être parfait (4 espaces)
    evolution_data = {
        "labels": labels,
...
def api_moyenne_classe_note(note_id):
    ...
    # Toutes les notes de la même matière pour tous les étudiants
    notes_matiere = Note.query.filter_by(matiere=note.matiere).all()
    if not notes_matiere:
        return jsonify({"moyenne_classe": None, "nb_eleves": 0})

    tp = sum(n.valeur * n.coefficient for n in notes_matiere)
    tw = sum(n.coefficient for n in notes_matiere)
...
def etudiant_edt():
    ...
    # Attribution d'une couleur unique par matière
    matieres_uniques = sorted(set(c.matiere for c in cours_list))
...
def etudiant_dm_passer(dm_id):
    ...
    if now < dm.date_debut:
        abort(403)  # Le DM n'est pas encore ouvert
    ...
    if request.method == "POST":
        ...
        for q in questions:
            ...
            if rep:
                ...
                rep = Reponse(soumission_id=soumission.id, question_id=q.id, valeur=val)
                db.session.add(rep)
...
def etudiant_devoirs():
    ...
    # 2. Récupération des IDs déjà faits pour cet utilisateur (Persistance)
    suivis = DevoirSuivi.query.filter_by(user_id=current_user.id, est_fait=True).all()
    faits_ids = [s.devoir_id for s in suivis]

    # 3. Groupement par date pour l'affichage "Cahier de texte"
...
def create_conversation():
    ...
    # Ajout automatique de l'utilisateur actuel dans la liste
...
def generer_token_jitsi(user_nom, user_email, user_avatar, room_name):
    ...
    # Dans un environnement de prod, récupère ceci via os.environ.get('JITSI_SECRET')
    jitsi_secret = os.environ.get("JITSI_SECRET", "secret_de_dev_temporaire")
    app_id = "skaolink_app"
...
def prof_dm_create():
    if request.method == "POST":
        ...
        for i in range(len(textes)):
            q_texte = textes[i]
            q_type = types[i]
            q_reponse = reponses[i]
            q_opts = options[i] if i < len(options) else ""
...
def prof_student_stats(class_id, student_id):
    ...
    if etudiant.classe_id != class_id:
        abort(403)

    notes = Note.query.filter_by(etudiant_id=student_id).all()
    absences = Absence.query.filter_by(etudiant_id=student_id).all()
...
def prof_add_cours():
    ...
    nouveau_cours = Cours(
        matiere=matiere,
        professeur=current_user.email,  # On utilise l'email du prof comme identifiant
        salle=salle,
        jour=int(jour),
...
def init_db():
    try:
        ...
        conv_globale.users.extend(tous_les_utilisateurs)
        db.session.add(conv_globale)
        db.session.flush()
        ...
        # 9. COMMIT FINAL
        db.session.commit()
        return "✅ Base de données initialisée ! 2 classes, 4 utilisateurs, Agenda et Chat WhatsApp prêts."
